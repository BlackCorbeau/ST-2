...
TEST(CircleTest, SetFerenceValid) {
  ...
  EXPECT_NEAR(c.getArea(), PI * expected_radius * expected_radius, EPSILON);
}
...
TEST(CircleTest, SetAreaValid) {
  ...
  EXPECT_NEAR(c.getFerence(), 2 * PI * expected_radius, EPSILON);
}
...
TEST(PoolTest, StandardValues) {
  PoolCost costs = calculatePoolCosts(3.0, 1.0, 1000.0, 2000.0);

  double expected_walkway_area = PI * 7.0;
  double expected_walkway_cost = expected_walkway_area * 1000.0;

  double expected_fence_length = 2 * PI * 4.0;
  double expected_fence_cost = expected_fence_length * 2000.0;

  EXPECT_NEAR(costs.walkway_cost, expected_walkway_cost, EPSILON);
  EXPECT_NEAR(costs.fence_cost, expected_fence_cost, EPSILON);
}

TEST(PoolTest, ZeroWidthWalkway) {
  PoolCost costs = calculatePoolCosts(3.0, 0.0, 1000.0, 2000.0);

  EXPECT_NEAR(costs.walkway_cost, 0.0, EPSILON);
  EXPECT_NEAR(costs.fence_cost, 2 * PI * 3.0 * 2000.0, EPSILON);
}

TEST(PoolTest, DifferentPoolRadius) {
  double radius = 5.0;
  PoolCost costs = calculatePoolCosts(radius, 1.0, 1000.0, 2000.0);

  double expected_walkway_area =
      PI * ((radius + 1) * (radius + 1) - radius * radius);
  EXPECT_NEAR(costs.walkway_cost, expected_walkway_area * 1000.0, EPSILON);
  EXPECT_NEAR(costs.fence_cost, 2 * PI * (radius + 1) * 2000.0, EPSILON);
}

TEST(PoolTest, DifferentConcretePrice) {
  double concrete_price = 1500.0;
  PoolCost costs = calculatePoolCosts(3.0, 1.0, concrete_price, 2000.0);

  double expected_walkway_area = PI * 7.0;
  EXPECT_NEAR(costs.walkway_cost, expected_walkway_area * concrete_price,
              EPSILON);
}

TEST(PoolTest, DifferentFencePrice) {
  double fence_price = 2500.0;
  PoolCost costs = calculatePoolCosts(3.0, 1.0, 1000.0, fence_price);

  double expected_fence_length = 2 * PI * 4.0;
  EXPECT_NEAR(costs.fence_cost, expected_fence_length * fence_price, EPSILON);
}

TEST(PoolTest, LargeWalkwayWidth) {
  PoolCost costs = calculatePoolCosts(3.0, 10.0, 1000.0, 2000.0);

  double expected_walkway_area = PI * (13.0 * 13.0 - 9.0);
  EXPECT_NEAR(costs.walkway_cost, expected_walkway_area * 1000.0, EPSILON);
  EXPECT_NEAR(costs.fence_cost, 2 * PI * 13.0 * 2000.0, EPSILON);
}

TEST(PoolTest, VerySmallPool) {
  PoolCost costs = calculatePoolCosts(0.1, 1.0, 1000.0, 2000.0);

  double expected_walkway_area = PI * (1.1 * 1.1 - 0.1 * 0.1);
  EXPECT_NEAR(costs.walkway_cost, expected_walkway_area * 1000.0, EPSILON);
  EXPECT_NEAR(costs.fence_cost, 2 * PI * 1.1 * 2000.0, EPSILON);
}
